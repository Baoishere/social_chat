...
class LoginViewModel extends ChangeNotifier {
  ...
  login(BuildContext context) async {
    ...
    if (!form.validate()) {
      validate = true;
      notifyListeners();
      showInSnackBar('Vui lòng sửa lỗi màu đỏ trước khi gửi.', context);
    } else {
      loading = true;
      notifyListeners();
      try {
  ...
  forgotPassword(BuildContext context) async {
    ...
    if (Validations.validateEmail(email) != null) {
      showInSnackBar(
          'Vui lòng nhập email hợp lệ để đặt lại mật khẩu.', context);
    } else {
      try {
        await auth.forgotPassword(email!);
        showInSnackBar(
            'Vui lòng kiểm tra email của bạn để biết hướng dẫn '
            'để thiết lập lại mật khẩu của bạn',
            context);
      } catch (e) {
